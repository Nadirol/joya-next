...
{ id: string, image: string | string[], duration: { days: number, nights: number }, 
    ...
    return (
        <Link href={`${i18n?.language}/tours/${id}`} className="rounded shadow-card min-w-[200px] bg-white
        md:min-w-[330px] snap-start overflow-hidden top-0 hover:top-[-4px] relative transition-all">
            <div className="w-[200px] md:w-[330px] h-[100px] md:h-[200px] overflow-hidden flex items-center justify-center">
                <Image src={typeof image === 'string' ? image : image[0]} alt="tour preview image" width={330} height={200} 
                className="hover:scale-[1.1] transition-all duration-300 object-cover h-[200px]"/>
            </div>
